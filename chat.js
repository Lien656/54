<script src="https://cdn.jsdelivr.net/npm/@google/genai@0.14.0/dist/index.js"></script>

<script>
    // **ВЕСЬ КОД ЗАПУСКАЕМ ПОСЛЕ ЗАГРУЗКИ СТРАНИЦЫ (ГАРАНТИЯ!)**
    window.onload = function() {
        ...
        // === ПАТЧ iOS (ВСТРОЕННАЯ ЛОГИКА) ===
        function iosEnterPatch(){
            const ta = document.getElementById("user-input"); // Используем твой ID: user-input
            if (!ta) return;
            // Мягкий скролл к низу при фокусе, чтобы клавиатура не дергала
            ta.addEventListener("focus", () => {
                setTimeout(()=> window.scrollTo({ top: document.body.scrollHeight, behavior: "smooth" }), 150);
            }, { passive: true });
        }
        iosEnterPatch(); // Запускаем патч сразу!
        // =======================================
    ...
    }; // Конец window.onload (ГАРАНТИЯ ЗАПУСКА!)
